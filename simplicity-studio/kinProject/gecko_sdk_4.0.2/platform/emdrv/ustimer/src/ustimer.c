...
uint32_t USTIMER_GetElapsedTime()
{
  // Snapshot of counter
  uint64_t ticks = (uint64_t) TIMER_CounterGet(TIMER_DEV);

  // Convert which takes time
  uint64_t faktor = (uint64_t)freq;
  uint64_t denominator = ((uint64_t)1) << 32;
  uint64_t us_shifted_up = (denominator / faktor) * ticks * 1000000;
  return (uint32_t) (us_shifted_up >> 32);
}
